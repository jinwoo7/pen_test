<?php
  ...
  function csrf_token_tag() {
    // needs to set a token and put in an HTML tag
    $token = csrf_token();
  ...
  function csrf_token_is_recent() {
    ...
    // check for existing csrf_token
    if(!isset($_SESSION['csrf_token'])) {
      return false; 
    }

    // checking to see if the csrf token is expired
    return (($_SESSION['csrf_time'] + $recent_limit) >= time());
  }
